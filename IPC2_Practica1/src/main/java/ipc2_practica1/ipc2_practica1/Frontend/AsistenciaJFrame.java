 ...
 * @author helder
 */
public class AsistenciaJFrame extends FrameBase {
    ...
    public void agregarVentanillaEvento(JDesktopPane jframe, int x, int y) {
        String titulo = "Ventanilla de Asistencia";
    ...
    private void agregarComponentes() {
        ...
        try {
            participante = controlador.listaParticipantesEmail();
        ...
        List<String> codigoActividad;
        try {
            codigoActividad = controlador.listaCodigoActividad();
            codigoEvento.addItem("Seleccione codigo...");
            for (String evento1 : codigoActividad) {
        ...
        enviarFormularioJButton.addActionListener(e -> {
            try {
                ...
                if (!entidad[0].equals("Seleccione correo...") && !entidad[1].equals("Seleccione codigo...") ) {
                    controlador.insetarFormularioAsistencia(entidad[0],entidad[1]);
